...
class config:
	WIDTH=30#地图列数
	HEIGHT=50#地图行数
	blockLength=18#绘制画面时每一个节点方块的边长
...
class A_Search:#核心部分，寻路类
	...
	def cal_F(self,loc,boolean):
		# print('计算值：',loc)
		G=loc.father.G+loc.cost
		H=self.getEstimate(loc,boolean)
		F=G+H
	...
	def getAroundPoint(self,loc):#获取指定点周围所有可通行的点，并将其对应的移动消耗进行赋值。
		l=l=[(loc.x,loc.y+1,10),(loc.x+1,loc.y+1,14),(loc.x+1,loc.y,10),(loc.x+1,loc.y-1,14),(loc.x,loc.y-1,10),(loc.x-1,loc.y-1,14),(loc.x-1,loc.y,10),(loc.x-1,loc.y+1,14)]
		for i in l[::-1]:
	...
	def getAroundPoint1(self,loc):#获取指定点周围所有可通行的点，并将其对应的移动消耗进行赋值。
		l=[(loc.x,loc.y+1,10),(loc.x+1,loc.y,10),(loc.x,loc.y-1,10),(loc.x-1,loc.y,10)]
		for i in l[::-1]:
			if i[0]<0 or i[0]>=config.HEIGHT or i[1]<0 or i[1]>=config.WIDTH:
				l.remove(i)
		nl=[]
	...
	def addToOpen(self,l,father,boolean):#此次判断的点周围的可通行点加入到open列表中，如此点已经在open列表中则对其进行判断，如果此次路径得到的F值较之之前的F值更小，则将其父节点更新为此次判断的点，同时更新F、G值。
		for i in l:
			if i not in self.open:
				if i not in self.close:
					i.father=father
					self.open.append(i)
					r=self.cal_F(i,boolean)
					i.G=r['G']
					i.F=r['F']
			else:
				tf=i.father
				...
				if i.F>r['F']:
					...
					i.father=tf
	def getEstimate(self,loc,boolean):#H :从点loc移动到终点的预估花费
		if boolean: return (abs(loc.x-self.end.x)+abs(loc.y-self.end.y))*10
		return math.sqrt(abs(loc.x-self.end.x)**2+abs(loc.y-self.end.y)**2)*100

	def process(self,boolean):#使用yield将process方法变成一个生成器，可以逐步的对搜索过程进行处理并返回关键数据
		while True:
			self.count+=1
			...
			if tar==None:
				...
				if boolean:
					...
					aroundP=self.getAroundPoint1(tar)
				self.addToOpen(aroundP,tar,boolean)#把aroundP加入到open列表中并更新F值以及设定父节点
...
class GameBoard(QMainWindow):#可视化类，pyqt5进行编写。
	...
	def initUI(self):
		...
		self.button_start1=QPushButton("AStar改进",self)
		...
		self.label_display.setAlignment(Qt.AlignTop)
			#设置控件的尺寸和位置
		self.label_tips.resize(200,300)
		self.button_saveMap.resize(80,50)
		self.button_loadMap.resize(80,50)
		self.button_start1.resize(100,50)
		self.label_display.resize(230,400)
		self.button_clearWall.resize(200,30)


		self.label_tips.move(100+(config.WIDTH-1)*config.blockLength,0)
		self.label_display.move(100+(config.WIDTH-1)*config.blockLength,520)
		self.button_start.move(100+(config.WIDTH-1)*config.blockLength,300)
		self.button_clearSE.move(100+(config.WIDTH-1)*config.blockLength,350)
		self.button_clearWall.move(100+(config.WIDTH-1)*config.blockLength,400)
		...
		self.button_start1.move(230+(config.WIDTH-1)*config.blockLength,300)
			#给控件绑定事件
		...
		self.button_start1.clicked.connect(self.button_StartEvent1)
		#UI初始化完成
	...
	def button_StartEvent(self):
		...
		if self.startPoint!=None and self.endPoint!=None:
			...
			self.yi=self.search.process(True)
			self.addDisplayText('开始进行搜索')
	def button_StartEvent1(self):
		sender=self.sender()
		print(sender)
		if self.startPoint!=None and self.endPoint!=None:
			if self.centerTimer==None:
			...
			self.centerTimer.start(200,self)
			self.search=A_Search(point(self.startPoint[1],self.startPoint[0]),point(self.endPoint[1],self.endPoint[0]),self.Map)
			self.yi=self.search.process(False)
			self.addDisplayText('开始进行搜索')
